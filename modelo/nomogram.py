...
def plot_prob(ax, title, x_point, prob, threshold=None,
             ...
             xtick_para = {"fontsize": 12,"fontfamily": "Times New Roman", "fontweight": "bold"},
...
def generate_xtick(range_, type_, mini, maxi, point, total_point):
    ...
    if type_ in ["nominal", "ordinal"]:
        ...
        if (range_%10 == 0) and (ran/(range_/5+1) < 0.07):
            ...
            xticklabels = [f"{label:.2f}" for label in np.linspace(mini, maxi, num + 1)]
        elif range_%5 == 0:
            num = int(range_/5)
            xticks = list(np.linspace(0, point/total_point, num+1))
            xticklabels = [f"{label:.2f}" for label in np.linspace(mini, maxi, num + 1)]
            ...
            xticklabels = [f"{label:.2f}" for label in np.linspace(mini, maxi, 6)]
    elif range_ <= 200:
        if range_%10 == 0:
            num = int(range_/10)
            xticks = list(np.linspace(0, point/total_point, num))
            xticklabels = [f"{label:.2f}" for label in np.linspace(mini, maxi, num)]
        else:
